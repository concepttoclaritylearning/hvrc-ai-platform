...
function SupportEmail() {
  const [supportEmail, setSupportEmail] = useState(paths.mailToHVRC());
  const { t } = useTranslation();
...
function HoldToReveal({ children, holdForMs = 3_000 }) {
  ...
  const [showing, setShowing] = useState(
    window.localStorage.getItem(
      "HVRC.AI_experimental_feature_preview_unlocked"
    )
  );
...
function AppVersion() {
  ...
  return (
    <Link
      to={`https://github.com/HVRC-Labs/hvrc-ai/releases/tag/v${version}`}
      target="_blank"
      rel="noreferrer"
